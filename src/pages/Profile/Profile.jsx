    ...
    return (
        <div className="container mx-auto">
            ...
            <div className="flex justify-between">
                <h1 className="lg:text-4xl text-3xl font-head text-[#080808] font-bold">DataList</h1>
                <Link to="/addNewSpot">
                ...
                <table className="table">
                    ...
                    <tbody>
                        {cards.map((cards, index) => (
                            <tr key={cards.id} className={index % 2 === 0 ? "bg-base-200" : ""}>
                                <th>{index + 1}</th>
                                <td>{cards.tourist_spot_name}</td>
                                <td>{cards.country_name}</td>
                                <td><Link to={`dataUpdate/${cards._id}`}><button className="btn btn-accent">Edit</button></Link></td>
                                <td><button className="btn btn-error" onClick={() => handleDelete(cards._id)}>Delete</button></td>
                            </tr>
                        ))}
                    </tbody>
                </table>
