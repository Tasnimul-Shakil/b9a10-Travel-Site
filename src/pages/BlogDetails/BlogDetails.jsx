    ...
    useEffect(() => {
        const fetchBlogData = async () => {
            try {
                ...
                const foundBlog = data.find(item => item.blog_id === parseInt(blog_id));
                setBlog(foundBlog);
            } catch (error) {
        ...
        <div className="container mx-auto">
            ...
            <div className="text-sm breadcrumbs">
                <ul>
                    ...
                    <li>
                        <Link to="/AllBlogs"><a className='flex items-center'> All Blogs</a></Link>
                    </li> 
                    <li>Blog Details</li> 
                </ul>
            </div>
            {blog && (
            <>
            <img className="w-full h-[22rem] rounded-2xl" src={blog.cover_image} alt="" />
            <div className="pl-4 pr-4 lg:px-24 ">
                ...
                <button className=" underline font-medium ">{blog.categories} </button>
                <p className="">{blog.time}</p>
                </div>
                <div className="relative py-2 flex items-center gap-x-4">
                    <img src={blog.author_image} alt="" className="h-10 w-10 rounded-full bg-gray-50" />
                ...
                <p className=" text-[#747474] italic leading-loose ">{blog.subtitle1_description}</p>
                <div className='grid pt-12 lg:grid-cols-2 gap-1 grid-cols-1'>
                    <div>
                        ...
                        <img className=' rounded-lg lg:w-full' src={blog.image_big} alt="" /> 
                        </div>
                    </div>
                    <div className='grid grid-row-2 gap-2'>
                        <div className='relative overflow-hidden bg-cover bg-no-repeat rounded-lg'>
                            <img className='  rounded-lg' src={blog.image_small_1} alt="" /> 
                        </div>
                        <div className='relative overflow-hidden bg-cover bg-no-repeat rounded-lg'>
                            <img className=' rounded-lg' src={blog.image_small_2} alt="" />
                        </div>
                    </div>
                </div>
                <h1 className=" text-2xl font-bold pt-12 pb-4">{blog.subtitle2}</h1>
                <p className=" text-[#747474] italic leading-loose">{blog.subtitle2_description}</p>
                <h1 className=" text-2xl font-bold pt-12 pb-4">{blog.subtitle3}</h1>
                <p className=" text-[#747474] italic leading-loose">{blog.subtitle3_description}</p>
            </div>
            <div className=" lg:px-24 max-w-6xl mx-auto pt-8">
               <div className="flex justify-between">
                ...
                {blog.comments.map((comment, index) => (
                    <div key={index} className="relative py-4 border-b-2 flex items-start gap-10">
                        <img src={comment.comment_avater_image} alt="" className="h-20 w-20 rounded-full bg-gray-50" />
                        <div className="text-sm leading-6">
                ...
                <form className="mx-auto mt-16  sm:mt-20">
                    ...
                    <button
                        type=""
                        className=" w-1/2 btn px-4 border-0 font-semibold text-white bg-[#7065F0] hover:bg-[#cce65b] hover:text-black"
                    >
                        Lets talk
                    </button>
                    </div>
            ...
            <div className='py-10'>
                ...
                {
                        cards.slice(0,dataLength).map(aCards => <Blogs
                            key={aCards.id}
                            card={aCards}
                            ></Blogs>)
                    }
